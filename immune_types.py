...
only one grow per bacteria- bad= multiple, good= one
17/10/24
Started: 11:20
"""
...
cycle_steps = 28
TRIALS = 1
INITIAL_IMMUNE_CELLS = 5
INITIAL_GOOD = 50
INITIAL_BAD = 50
INITIAL_GLYCOGEN = 200
IMMUNE_INTERACTION_RANGE = 1
GLYCOGEN_RANGE = 0.2
...
class Microbe:
    ...
    def move(self, bacteria, alignment, index):
        ...
        if abs(new_location[1]) > 9 or new_location[1] < 0:
                        ...
                        #print("hi")
                            if cell in bacteria["Good"]["Cells"][cell.species]:
                                if calculate_distance(self.location, cell.location) <= IMMUNE_INTERACTION_RANGE:
                                    ...
                                    self.kills += 1
                                    #print(self.kills)
                                    if self.kills > NEUTROPHIL_KILL_LIMIT:
                                        #print(self)
                                        immune_cells['Cells'].remove(self)
                                        immune_cells["Total"][-1] -= 1
                        ...
                        #print("hi")
                            if cell in bacteria["Good"]["Cells"][cell.species]:
                                if calculate_distance(self.location, cell.location) <= IMMUNE_INTERACTION_RANGE:
                                    ...
                                    self.kills += 1
                                    #print(self.kills)
                                    if self.kills > MACROPHAGE_KILL_LIMIT:
                                        #print(self)
                                        immune_cells['Cells'].remove(self)
                                        immune_cells["Total"][-1] -= 1
                        ...
                        #print("hi")
                            if cell in bacteria["Good"]["Cells"][cell.species]:
                                if calculate_distance(self.location, cell.location) <= IMMUNE_INTERACTION_RANGE:
                                    ...
                                    self.kills += 1
                                    #print(self.kills)
                                    if self.kills > NK_CELL_KILL_LIMIT:
                                        #print(self)
                                        immune_cells['Cells'].remove(self)
                                        immune_cells["Total"][-1] -= 1
...
def cytokine_level(bacteria):
    total_good_bacteria = 0
    total_bad_bacteria = 0
    for i in range(1,5):
        total_good_bacteria += bacteria["Good"]["Total"][f"Good_Bacteria_{i}"][-1]
        if i < 3:
            total_bad_bacteria += bacteria["Bad"]["Total"][f"Bad_Bacteria_{i}"][-1]
    total_bacteria = total_good_bacteria + total_bad_bacteria  
    
    try:
        bad_bacteria_proportion = total_bad_bacteria/total_bacteria
...
def pH(system, bacteria, step):
    ...
    if step < 7:
        ...
        if total_good_bacteria/total_bacteria > 0.6:
            pH_p = system["pH"][-1]
            pH_v = random.uniform(3.5, 4.5)
            vol_p = 1*10**-3
            vol_v = 0.3*10**-3
    ...
    new_pH = solution_pH_calc(pH_p, pH_v, pH_i, vol_p, vol_v, vol_i)        
    return new_pH 
...
def initial_state(system, Oestrogen, progesterone, step, bacteria):
    ...
    system["Cytokine_Level"].append(cytokine_level(bacteria))
...
def new_immune_cells(system, step, immune_cells):
    ...
    if system['Oestrogen'][-1] > 0.6:
        no_new_immune_cells = int(5 *(1+system['Cytokine_Level'][-1]))
        for _ in range(no_new_immune_cells):
            immune_type = random.choice(["Macrophage", "Neutrophil", "NK"])
            if immune_type == "Macrophage":
                new_immune_cell = Macrophage(species = "Macrophage", location=(random.uniform(
                    0, 9), random.uniform(0, 9)), kills = 0)
                #print("Macrophage")
                
            elif immune_type == "Neutrophil":
                new_immune_cell = Neutrophil(species = "Neutrophil", location=(random.uniform(
                    0, 9), random.uniform(0, 9)), kills = 0)
                #print("Neutrophil")
            elif immune_type == "NK":
                new_immune_cell = NK_Cell(species = "NK_Cell", location=(random.uniform(
                    0, 9), random.uniform(0, 9)), kills = 0)
            immune_cells['Cells'].append(new_immune_cell)
            immune_cells["Total"][-1] +=1
        #print(no_new_immune_cells)
                
    elif system["Progesterone"][step] > 0.6:
        pass
        
    elif system["Oestrogen"][-1] > 0.1:
        no_new_immune_cells = int(2 *
                                  (1+system['Cytokine_Level'][-1]))
        for _ in range(no_new_immune_cells):
            immune_type = random.choice(["Macrophage", "Neutrophil", "NK"])
            if immune_type == "Macrophage":
                new_immune_cell = Macrophage(species = "Macrophage", location=(random.uniform(
...
def plotting(system, bacteria, immune_cells, days):
    ...
    plt.plot(range(1, len(system["Oestrogen"]) + 1), system["Oestrogen"],
             label='Oestrogen Levels')
