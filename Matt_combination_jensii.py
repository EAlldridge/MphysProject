...
class Tumour:
    ...
    def stage_change(self):
        ...
        if self.grade == 1 and self.stage < 2:
            prob = random.uniform(0,1)
            if prob <0.5:
                self.stage = 1
            else:
                self.stage = 2
        elif self.grade == 2 and self.stage < 3:
            prob = random.uniform(0,1)
            if prob <0.3:
                self.stage = 1
            elif prob < 0.6:
                ...
                self.stage = 3
        elif self.grade == 3 and self.stage < 3:
            prob = random.uniform(0,1)
            if prob <0.3:
                self.stage = 3
            elif prob < 0.6:
                ...
                self.stage = 4
        elif self.grade == 4 and self.stage < 4:
            prob = random.uniform(0,1)
            if prob <0.4:
                self.stage = 3
            else:
...
def handle_microbes(state, microbe_trackers, glycogen_objects, bacteriocin_objects, bacteriocin_objects_E, ca125_objects):
    """
...
def simulation_loop():
        ...
        for test in range(TRIALS):
            ...
            for cycle in range(i):
                ...
                for step in range(DAYS):
                    ...
                    handle_microbes(
                        current_state, microbe_trackers, glycogen_objects, bacteriocin_objects, bacteriocin_objects_E, ca125_objects)
    ...
    return environment, microbe_trackers, good_bacteria_proportion_tracker, good_bacteria_std_tracker, external_trackers, ca125_objects, tumour_sizes


def main():
    environment, microbe_trackers, good_bacteria_proportion_tracker, good_bacteria_std_tracker, external_trackers, ca125_objects, tumour_sizes = simulation_loop()
