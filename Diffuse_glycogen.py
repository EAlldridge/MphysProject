...
class Tumour:
    ...
    def secrete_ca125(self, ca125_objects):
        ...
        if "grade_4" in self.grade:
            ca125_level = random.randint(5, 10)
        elif "grade_3" in self.grade:
            ca125_level = random.randint(3, 7)
    ...
    def increase_size(self, tumour_sizes):
        ...
        if self.subtype == "high_grade":
            V_0 = 1
            V_max = 28
            growth_rate1 = 0.01  # Steeper growth rate
            log_bracket = np.log(
                (V_max/V_0)*(1-math.exp(-growth_rate1*self.time)))
        ...
        # Slower growth for low-grade tumours
            ...
            self.size = 20 / (1 + math.exp(-growth_rate * (self.time - 100))
                              ) - 1 / (1 + math.exp(5 * growth_rate))
...
def calculate_local_concentration(location, glycogen_objects, radius=1):
    count = 0
    for glycogen in glycogen_objects:
        if glycogen != location and calculate_distance(location, glycogen.location) <= radius:
            count += 1
    return count
...
def tumour_interaction(tumour, ca125_levels, tumour_sizes):
    ...
    tumour.secrete_ca125(ca125_levels)
...
def plotting(enviroment, microbe_trackers, good_bacteria_proportion_tracker, external_trackers, ca125_levels, tumour_sizes):
...
'''
    ...
    plt.subplot(2, 1, 1)
    plt.plot(range(1, len(tumour_sizes) + 1), ca125_levels,
    ...
    plt.plot(range(1, len(tumour_sizes) + 1), tumour_sizes,
             label='tumour size over time', linestyle='--', color='blue')
...
def main():
    enviroment, microbe_trackers, good_bacteria_proportion_tracker, external_trackers, ca125_objects, tumour_sizes = simulation_loop()
    ...
    return enviroment, microbe_trackers, good_bacteria_proportion_tracker, ca125_objects, tumour_sizes


enviroment, microbe_trackers, good_bacteria_proportion_tracker, ca125_objects, tumour_sizes = main()
