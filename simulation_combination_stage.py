...
class Tumour:
    def __init__(self, subtype, grade, stage, size, asymptomatic, time = 0):
    ...
    def secrete_ca125(self, ca125_objects):
        ...
        if self.stage == 4:
            ...
            ca125_level += 0  # Handle unexpected stages gracefully
    ...
    def stage_change(self, VEGF):
        if VEGF > 0.01 and self.stage < 1:
            self.stage = 1
        if VEGF > 0.02 and self.stage < 2:
            self.stage = 2
        if VEGF > 0.5 and self.stage < 3:
            self.stage = 3
...
def create_tumour(cycle, step):
    ...
    if cycle == CYCLE_START and step == STEP_START:
        ...
        tumour = Tumour(subtype, grade='grade_1', stage = 1, size=0, asymptomatic = False)
        return tumour
    else:
        return None
        ...
        for test in range(TRIALS):
            ...
            for cycle in range(i):
                ...
                for step in range(DAYS):
                    ...
                    if tumour_created:
                        tumour_interaction(tumour, ca125_objects, tumour_sizes, environment["VEGF_level"])
                    else:
