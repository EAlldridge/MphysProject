...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 21 09:29:19 2024
...
NUMBER_IMMUNE_CELLS = 5  # scaling for number of immune cells made per step
...
class Tumour:
    def __init__(self, subtype, grade, size, time=0):
        self.subtype = subtype
        self.grade = grade
        self.size = size
        self.time = time
        self.vegf_concentration = 0

    def secrete_ca125(self, ca125_objects):
        # Create a new list for the current level of CA125 secretion
        produced = []

        # Determine the CA125 secretion level based on tumour grade
        if "grade_4" in self.grade:
            ...
            ca125_level = 0  # Handle unexpected grades gracefully
    ...
    def secrete_vegf(self):
        if self.size > 3 and self.size < 10:
            self.vegf_concentration = random.uniform(0, 0.03)
        else:
            self.vegf_concentration = 0
        return self.vegf_concentration
    ...
    def increase_size(self, tumour_sizes):
        ...
        # Slower growth for low-grade tumours
            ...
            growth_rate = 0.001  # Gentler growth rate
        ...
        log_bracket = np.log(
            (V_max/V_0)*(1-math.exp(-eff_growth_rate*self.time)))
        self.size = V_0 * math.exp(log_bracket)
...
def initialize_state(estrogen_level, progesterone_level, good_bacteria_count, bad_bacteria_count, glycogen_objects, ca125_objects, inflammation_objects, tumour, step, pH_level):
    ...
    if tumour is not None:
        ca125_objects = tumour.secrete_ca125(ca125_objects)
        ca125_levels = len(ca125_objects)
    else:
        ca125_objects = []
        ca125_levels = 0  # Or handle appropriately when no tumour is present
...
def handle_microbes(state, microbe_trackers, glycogen_objects):
    ...
    if not any(microbe_trackers['good object'].values()) or not any(microbe_trackers['bad object'].values()):
        ...
        initialize_bacteria(
            microbe_trackers, num_good_bacteria=10, num_bad_bacteria=20)
...
def create_tumour(cycle, step):
    ...
    if cycle == CYCLE_START and step == STEP_START:
        ...
        tumour = Tumour(subtype, grade='grade_1', size=0)
        return tumour
    else:
        return None


def tumour_interaction(tumour, ca125_levels, tumour_sizes):
...
'''
    ...
    # Use 1-based indexing for simulation steps
    ...
    plt.plot(range(1, len(environment['ca125_level']) + 1), environment['ca125_level'],
             label='CA-125 Levels', linestyle='--', color='blue')
    ...
    # Plot tumor sizes
    plt.subplot(2, 1, 2)
    plt.plot(range(1, len(tumour_sizes) + 1), tumour_sizes,
             label='Tumor size over time', linestyle='--', color='blue')
...
def simulation_loop():
        ...
        for test in range(TRIALS):
            ...
            for cycle in range(i):
                ...
                for step in range(DAYS):
                    ...
                    if tumour_created:
                        tumour_interaction(tumour, ca125_objects, tumour_sizes)
                    else:
                        tumour_sizes.append(0)

                    # Initialize the state for the current step, passing existing glycogen_objects
                    current_state = initialize_state(
                        estrogen_level, progesterone_level, good_bacteria_count, bad_bacteria_count, glycogen_objects, ca125_objects, inflammation_objects, tumour if tumour_created else None, step, pH_levels
                    )
                    ...
                    environment["ca125_level"].append(
                        current_state["ca125_levels"] if tumour_created else 0
                    )
                    environment["ca125_objects"].append(
                        [CA for CA in current_state["ca125_objects"]
                         ] if tumour_created else []
                    )
    ...
    return environment, microbe_trackers, good_bacteria_proportion_tracker, external_trackers, ca125_objects, tumour_sizes


def main():
    environment, microbe_trackers, good_bacteria_proportion_tracker, external_trackers, ca125_objects, tumour_sizes = simulation_loop()
