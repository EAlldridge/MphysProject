...
GLYCOGEN_SCALE = 10000  # maximum number of new glycogen clumps generated per step
GLYCOGEN_CAPACITY = 30  # The number of bacteria that a glycogen cluster can support
...
def h202_level(good_bacteria_count, bad_bacteria_count, estrogen_level):
    if good_bacteria_count > 2*bad_bacteria_count or estrogen_level > 0.7:
        return random.uniform(0.5, 1)
    elif good_bacteria_count > bad_bacteria_count or estrogen_level > 0.4:
...
def handle_microbes(state, microbe_trackers, glycogen_objects):
    ...
    if not any(microbe_trackers['good object'].values()) or not any(microbe_trackers['bad object'].values()):
        ...
        initialize_bacteria(
            microbe_trackers, num_good_bacteria=50, num_bad_bacteria=100)
    ...
    for bacterium in all_bacteria:
        if bacterium.alive:
            ...
            if "Good_Bacteria" in bacterium.species:
                ...
                if state['hydrogen_peroxide_level'] > 2*state['iron_level']:
                ...
                if 3*state['iron_level'] > state['hydrogen_peroxide_level']:
                    new_bacterium = bacterium.replicate(glycogen_objects)
                else:
                    continue
