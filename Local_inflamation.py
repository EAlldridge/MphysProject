...
class Inflammation:
    ...
    def inflammation_calc(self, CA125, cytokines):
        for obj in CA125:
            if self.location[0] =< CA125.location[0] =< self.location[0]+1 and self.location[1] =< CA125.location[1] =< self.location[1]+1:
...
class Tumour():
    def __init__(self, stage, grade, size, age):
        self.stage = stage
        self.grade = grade
        self.size = size
        self.age = age
    ...
    def secretions(self):
        ...
        if self.size > 100:
            ...
            add = 4
        if self.stage == 1:
            r = random.randint(1,2) #50% low grade have elevated
            if r == 1:
                return 10+add
            else:
                return 1+add
...
def initialize_state(estrogen_level, progesterone_level, good_bacteria_count, bad_bacteria_count, glycogen_objects, step, pH_level, tumour, CA125_objects):
    ...
    CA125_objects = CA_calc(estrogen_level, progesterone_level, tumour, CA125_objects)
    inflammation_objects = inflammation_calc()
...
def CA_calc(estrogen, progesterone, tumour, CA125_objects):
    current_CA = natural_CA125(estrogen, progesterone)
    current_CA += Tumour.secretions(tumour)
    for _ in range(current_CA):
        CA = CA125((random.randint(0,SIDE_LENGTH), random.randint(0,SIDE_LENGTH)))
...
def simulation_loop():
        ...
        for cycle in range(NUM_CYCLES):
            ...
            for step in range(DAYS):
                ...
                current_state = initialize_state(
                    estrogen_level, progesterone_level, good_bacteria_count, bad_bacteria_count, glycogen_objects, step, pH_levels, tumour, CA125_objects)
                ...
                environment["Inflammation"].append(unit for unit in current_state["Inflammation"])
    ...
    return environment, microbe_trackers, good_bacteria_proportion_tracker, tumour_tracker


def main():
    environment, microbe_trackers, good_bacteria_proportion_tracker, tumour_tracker = simulation_loop()
